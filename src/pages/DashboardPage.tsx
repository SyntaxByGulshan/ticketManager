...
export default function DashboardPage() {
  ...
  const allComments: { ticketId: string; comment: string; commentTime: string }[] = [];
  ...
  ticketsArray.forEach((ticket) => {
    ...
    if (ticket.isDeleted && (ticket as any).deletedAt) {
      allActivity.push({
        message: `Ticket #${ticket.id} deleted`,
        date: new Date((ticket as any).deletedAt),
        type: "delete",
      });
    }
    if (ticket.status === "Resolved") {
      allActivity.push({
        ...
        date: new Date(ticket.createdAt), // better if resolvedAt exists
        type: "resolve",
      });
    }
  });
        ...
        {recentComments.length > 0 ? (
          <ul className="space-y-3">
            {recentComments.map((c, i) => (
              <li key={i} className="border-b last:border-b-0 pb-2 text-sm text-gray-700">
                <span className="font-medium text-gray-900">Ticket #{c.ticketId}:</span> {c.comment}
                <span className="block text-xs text-gray-500">{c.commentTime}</span>
              </li>
            ))}
          </ul>
        ) : (
...
function StatCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: number }) {
  return (
    <div className="bg-white p-4 rounded-xl shadow-sm flex flex-col items-center justify-center border">
